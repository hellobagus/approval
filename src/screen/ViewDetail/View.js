...
import { getOtorisasi } from "../../actions/MenuActions";
...
function ViewDetail(props) {
    ...
    const {
         ...
         descs,
         approval_user_name,
         approval_user, module : modules } = props.navigation.state.params.data;

    const user = useSelector(state => getUser(state));
    const otorisasi = useSelector(state => selectOtorisasi(state));

    const getOtorisasis = useCallback(() => dispatch(getOtorisasi(entity_cd,doc_no,modules,)), [
        dispatch
    ]);
          ...
          <Tabs tabBarUnderlineStyle={styles.tabBorder}>
            ...
            >
              ...
              <List style={styles.infoTab}>
                ...
                <ListItem style={styles.infoItem}>
                  <View>
                    <Text style={styles.infoDesc}>Staff ID : {request_staff_id}</Text>
                  </View>
                </ListItem>
                <ListItem style={styles.infoItem}>
                  <View>
            ...
            <View style={styles.itemRow}>
                <View style={styles.itemOverview}>
                    <View>
                        <Text style={styles.itemLocation}>{data.approval_user_name}</Text>
                        <Text style={styles.itemLocation}>{data.approval_name}</Text>
                    </View>
                </View>
                <View style={styles.itemRight}>
                    <Text style={styles.itemDate}>Status</Text>
                    <View style={styles.itemBtn1}>
                    <Text style={styles.itemEntity}>{data.approval_status}</Text>
                    </View>
